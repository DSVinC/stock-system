...
async function bootstrap(db) {
  ...
  await db.runPromise(`CREATE TABLE strategy_versions (
...
async function main() {
  ...
  `, ['测试账户', 50000, 50000, 50000, 0, 0, now, now]);
  ...
  const result = await executeConditionalOrder(orderResult.lastID, { price: 10.5 }, {}, { db });
  assert.equal(result.success, true, '执行应成功');

  const feedback = await db.getPromise(
    'SELECT * FROM execution_feedback WHERE conditional_order_id = ? AND event_type = ?',
    [orderResult.lastID, 'simulated_trade']
  );
  assert.ok(feedback, '应写入 simulated_trade 反馈');
  assert.equal(feedback.trade_id, 1, '应关联新建 trade_id');
  assert.equal(feedback.strategy_source, 'strategy_config');
  assert.equal(feedback.strategy_config_id, 101);
  assert.equal(feedback.strategy_config_name, '七因子高分策略（执行副本）');
  assert.equal(feedback.template_id, 88);
  assert.equal(feedback.template_name, '七因子模板');
  assert.equal(feedback.strategy_id, 'STRAT-001');
  assert.equal(feedback.strategy_version, 'ver-001');
  assert.equal(feedback.version_id, 'ver-001');
  assert.equal(feedback.report_id, 'REPORT-001');
  assert.equal(feedback.action, 'buy');
  assert.equal(feedback.quantity, 200);
  assert.equal(feedback.price, 10.5);
  assert.equal(feedback.amount, 2100);

  console.log('✅ conditional execution feedback test passed');
}

main().catch((error) => {
  console.error(`❌ conditional execution feedback test failed: ${error.message}`);
  process.exit(1);
});
